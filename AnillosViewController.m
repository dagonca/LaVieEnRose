...
#import "AnillosViewController.h"
#import "CollectionViewCell.h"
#import "InfoViewController.h"

@interface AnillosViewController ()

@end

@implementation AnillosViewController{
    NSArray *FotosArray;
    NSArray *FotosArray1;
    NSArray *FotosArray2;
    NSArray *FotosArray3;
...
- (void) retrieveFromParse {
    ...
    PFQuery *retrieveColors = [PFQuery queryWithClassName:@"Anillos"];
    ...
    [retrieveColors findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error) {
        if (!error) {
            ...
            precioArray2 = [[NSArray alloc] initWithArray:objects];
...
- (IBAction)SegmentControl:(id)sender {
    ...
    if(segment.selectedSegmentIndex == 1){
        PFQuery *retrieveColors = [PFQuery queryWithClassName:@"Anillos"];
        [retrieveColors orderByAscending:@"precio"];
        [retrieveColors findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error) {
            if (!error) {
                FotosArray = [[NSArray alloc] initWithArray:objects];
                precioArray = [[NSArray alloc] initWithArray:objects];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([segue.identifier isEqualToString:@"Info"]) {
        ...
        PFObject *PrecioObject = [precioArray2 objectAtIndex:indexPath.row];
        destViewController.StringPrecio =[PrecioObject objectForKey:@"precio"];
